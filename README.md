...
- `vercel.json` — configuración para desplegar directamente en Vercel (sin build; sirve los archivos estáticos tal cual, `outputDirectory` fijado a la raíz).

## Cómo publicarlo en Vercel

1. Sube esta carpeta a un repositorio de GitHub.
2. En [vercel.com](https://vercel.com), pulsa **Add New → Project** e importa ese repositorio.
3. En **Build & Development Settings**: Framework Preset **Other**, Build Command **vacío/None**, Output Directory **vacío o `.`** (no dejes "dist"). `vercel.json` ya fija `outputDirectory: "."` para evitar el error de "No Output Directory named dist".
