...
(function () {
  ...
  function animateCount(el) {
    ...
    if (reduce) { el.textContent = prefix + formatNum(target); return; }
    var dur = 1600, start = null;
    function step(ts) {
      if (!start) start = ts;
      var p = Math.min((ts - start) / dur, 1);
      var eased = 1 - Math.pow(1 - p, 3);
      el.textContent = prefix + formatNum(target * eased);
  ...
  function buildForm(key) {
    ...
    form.addEventListener('submit', function (e) {
      ...
      var mailto = 'mailto:laligapava@gmail.com?subject=' + encodeURIComponent(subject) + '&body=' + encodeURIComponent(body);
      window.location.href = mailto;

      mBody.innerHTML = '<div class="form-success"><div class="ic">✓</div><h3>' + f.success + '</h3><p>' + f.successMsg + '</p>' +
        '<p class="form-note" style="margin-top:16px">Se ha abierto tu correo con los datos listos para enviar a <b style="color:var(--orange)">laligapava@gmail.com</b>. Si no se abre, escríbenos directamente.</p>' +
