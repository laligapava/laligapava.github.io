...
(function () {
  ...
  document.addEventListener('keydown', function (e) { if (overlay && e.key === 'Escape' && overlay.classList.contains('open')) closeModal(); });
  ...
  /* ---------- Auto-fit t\u00edtulo patrocinadores en 1 l\u00ednea ---------- */
  ...
  function fitSponsTitle() {
    ...
    if (el && window.innerWidth <= 700) fitOneLine(el);
    else if (el) el.style.fontSize = '';
  }
  fitSponsTitle();
  window.addEventListener('resize', fitSponsTitle);
  if (document.fonts && document.fonts.ready) document.fonts.ready.then(fitSponsTitle);
  /* ---------- Barra de cookies ---------- */
  function initCookieBar() {
    var cookieBar = document.getElementById('cookieBar');
    if (!cookieBar) return;
    if (!localStorage.getItem('pava_cookies_ok')) {
      setTimeout(function () { cookieBar.classList.add('show'); }, 900);
    }
    var acceptBtn = document.getElementById('cookieAccept');
    if (acceptBtn) acceptBtn.addEventListener('click', function () {
      localStorage.setItem('pava_cookies_ok', '1');
